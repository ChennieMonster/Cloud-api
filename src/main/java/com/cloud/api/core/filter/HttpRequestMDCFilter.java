...
//    private static final String PARAMETERS_CONTENT = "requestParameters";  
    ...
    private void mdc(HttpServletRequest hsr) {  
        ...
        MDC.put(MDCConstants.REQUEST_ID_MDC_KEY, "[REQ "+StringUtils.remove(UUID.randomUUID().toString(),"-")+"]");
        MDC.put(MDCConstants.USER_NAME, hsr.getAttribute(MDCConstants.USER_NAME).toString());
        MDC.put(MDCConstants.REGION_ID, hsr.getAttribute(MDCConstants.REGION_ID).toString());
